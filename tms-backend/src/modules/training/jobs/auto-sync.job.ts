...
import { DiscordServer, Enrollment, Student, Teacher, Topic, TopicProblem, TopicStanding } from '../../../entities/index.js';
import {
  extractGymIdFromLink,
  fetchCodeforcesGymMetadata,
  fetchCodeforcesGymStandings,
  resolveCodeforcesCredentials,
  type CodeforcesCredentials,
} from '../../../integrations/codeforces/codeforces-api.service.js';
import { fetchDiscordGuildMetadata } from '../../../integrations/discord/discord-api.service.js';
...
async function fetchCodeforcesGymStandingsSafely(
  gymId: string,
  credentials: CodeforcesCredentials | null,
): Promise<Awaited<ReturnType<typeof fetchCodeforcesGymStandings>> | null> {
  try {
    return await fetchCodeforcesGymStandings(gymId, credentials);
  } catch (error) {
    console.warn(`[auto-sync] failed to sync standings for gym ${gymId}`, error);
    return null;
  }
}
...
async function syncTopicStanding(
  ...
  const standings = await fetchCodeforcesGymStandingsSafely(gymId, credentials);
  if (!standings) {
    return false;
  }
...
async function syncCodeforcesTopicsOnce(): Promise<void> {
  ...
  for (const topic of topics) {
    ...
    if (topic.gym_id === synced.gym_id && topic.title === synced.title) {
      if (shouldPullTopicStanding(topic, now) && await syncTopicStanding(topic, credentials, now)) {
        standingSyncCount += 1;
      }
      continue;
    }
  ...
  if (standingSyncCount > 0) {
    console.log(`[auto-sync] codeforces standings synced: ${standingSyncCount}`);
  }
}
