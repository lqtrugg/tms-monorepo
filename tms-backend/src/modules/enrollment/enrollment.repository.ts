...
import type { StudentBalanceSnapshot, StudentListFilters } from './enrollment.types.js';
...
export class EnrollmentRepository {
  ...
  createZeroBalanceSnapshot(): StudentBalanceSnapshot {
    return createZeroBalanceSnapshot();
  }
  ...
  listStudentsForTeacher(teacherId: number, filters: StudentListFilters): Promise<Student[]> {
    return listStudentsForTeacher(this.manager, teacherId, filters);
  }

  findActiveEnrollment(teacherId: number, studentId: number): Promise<Enrollment | null> {
    return findActiveEnrollment(this.manager, teacherId, studentId);
  }

  findActiveEnrollmentsByStudentIds(teacherId: number, studentIds: number[]): Promise<Enrollment[]> {
    return findActiveEnrollmentsByStudentIds(this.manager, teacherId, studentIds);
  }

  findLastEnrollment(teacherId: number, studentId: number): Promise<Enrollment | null> {
    return findLastEnrollment(this.manager, teacherId, studentId);
  }

  findRecentEnrollments(teacherId: number, studentId: number, take: number): Promise<Enrollment[]> {
    return findRecentEnrollments(this.manager, teacherId, studentId, take);
  }

  findDiscordServerByClass(teacherId: number, classId: number): Promise<DiscordServer | null> {
    return findDiscordServerByClass(this.manager, teacherId, classId);
  }
  ...
  loadBalanceSnapshots(
    ...
    return loadBalanceSnapshots(this.manager, teacherId, studentIds);
  }

  loadBalanceSnapshotForStudent(teacherId: number, studentId: number): Promise<StudentBalanceSnapshot> {
  ...
  codeforcesHandleExists(
    ...
    return codeforcesHandleExists(this.manager, codeforcesHandle, excludeStudentId);
  }
}
