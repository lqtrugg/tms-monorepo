...
import { getFinanceSummary, listStudentBalances, listTransactions } from '../../finance/index.js';
import {
  countActiveClasses,
  countActiveStudents,
  findClassesByIds,
  ...
  findTopicsByIds,
} from './academic-report.repository.js';
...
export async function getDashboardSummary(teacherId: number) {
  ...
  const financeSummary = await getFinanceSummary(teacherId, {
    from: monthStart,
    to: new Date(),
...
export async function getStudentLearningProfile(teacherId: number, studentId: number) {
  ...
  const { items: transactions } = await listTransactions(teacherId, { student_id: studentId });
