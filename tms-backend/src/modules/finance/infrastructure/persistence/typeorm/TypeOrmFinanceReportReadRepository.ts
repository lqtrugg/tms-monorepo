...
import type { FinanceReportReadRepository } from '../../../application/queries/FinanceReportReadRepository.js';
...
export class TypeOrmFinanceReportReadRepository implements FinanceReportReadRepository {
  ...
  findReportClasses(teacherId: number, classIds?: number[]) {
    return this.manager.getRepository(ClassOrmEntity).find({
      where: {
  ...
  async getFinanceSummary(
    ...
    const transactionTotals = studentIds.length === 0
      ? []
      : await this.manager.getRepository(TransactionOrmEntity)
        .createQueryBuilder('transaction')
    ...
    const feeTotals = studentIds.length === 0
      ? []
      : await this.manager.getRepository(FeeRecordOrmEntity)
        .createQueryBuilder('fee_record')
