...
export function dateOnlyToDate(value: string): Date {
  ...
  return vietnamDateTimeToUtcDate(year, month, day, 0, 0, 0, 0);
}
...
function nowStartOfDay(): Date {
  return vietnamStartOfDay(new Date());
}

function endOfDay(date: Date): Date {
  return vietnamEndOfDay(date);
}

function addDays(date: Date, amount: number): Date {
  return addVietnamDays(date, amount);
}

function dateOnlyString(date: Date): string {
  return vietnamDateOnlyString(date);
}
...
async function assertNoUpcomingSessionOverlapForSchedules(
  ...
  for (const schedule of schedules) {
    for (let cursor = startDate; cursor <= endDate; cursor = addDays(cursor, 1)) {
      if (toVietnamDateParts(cursor).dayOfWeek !== schedule.day_of_week) {
        continue;
      }
...
async function generateSessionsForSchedule(
  ...
  for (let cursor = startDate; cursor <= endDate; cursor = addDays(cursor, 1)) {
    ...
    const dateOnly = dateOnlyString(cursor);
...
function sessionMatchesSchedule(session: Session, schedule: ClassSchedule): boolean {
  const scheduledTime = vietnamTimeString(session.scheduled_at);

  return toVietnamDateParts(session.scheduled_at).dayOfWeek === schedule.day_of_week
...
function combineSessionWriterDateWithEndTime(date: Date, endTime: string): Date {
  ...
  return vietnamDateTimeToUtcDate(parts.year, parts.month, parts.day, hours, minutes, seconds, 0);
}
