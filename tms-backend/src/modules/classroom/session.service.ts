...
function requireSessionById(manager: EntityManager, sessionId: number): Promise<Session> {
  return manager.getRepository(Session).findOneBy({ id: sessionId }).then((session) => {
    if (!session) {
...
async function syncFeeRecordByAttendance(
  ...
  await financeFeeSync.syncAttendanceFeeRecord(manager, {
    teacherId,
...
export async function listSessionAttendance(teacherId: number, sessionId: number) {
  const session = await requireSessionById(AppDataSource.manager, sessionId);
...
export async function materializeSessionAttendance(teacherId: number, sessionId: number): Promise<{
  ...
  return AppDataSource.transaction(async (manager) => {
    ...
    const classEntity = await requireClassById(manager, session.class_id);
...
export async function resetSessionAttendance(teacherId: number, sessionId: number): Promise<void> {
  ...
  await financeFeeSync.cancelFeeRecordsForSessions(AppDataSource.manager, teacherId, [session.id]);
}

export class SessionService {
  constructor(private readonly db?: DbContext) {}

  listSessionAttendance(teacherId: number, sessionId: number): ReturnType<typeof listSessionAttendance> {
    void this.db;
    return listSessionAttendance(teacherId, sessionId);
  }
  ...
  upsertSessionAttendance(
    ...
    studentId: number,
    input: UpsertAttendanceInput,
  ): ReturnType<typeof upsertSessionAttendance> {
    return upsertSessionAttendance(teacherId, sessionId, studentId, input);
  }

  upsertBotSessionAttendance(
    teacherId: number,
    sessionId: number,
    studentId: number,
  ): ReturnType<typeof upsertBotSessionAttendance> {
    return upsertBotSessionAttendance(teacherId, sessionId, studentId);
  }

  materializeSessionAttendance(
    teacherId: number,
    sessionId: number,
  ): ReturnType<typeof materializeSessionAttendance> {
    return materializeSessionAttendance(teacherId, sessionId);
  }

  listAttendanceRecords(
    teacherId: number,
    filters: Parameters<typeof listAttendanceRecords>[1],
  ): ReturnType<typeof listAttendanceRecords> {
    return listAttendanceRecords(teacherId, filters);
  }

  resetSessionAttendance(teacherId: number, sessionId: number): Promise<void> {
    return resetSessionAttendance(teacherId, sessionId);
  }
}
